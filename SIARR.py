...
def mostrar_modulo_ia():
    ...
    if st.button("🚀 Iniciar Análisis de Red Neuronal", type="primary", use_container_width=True):
        ...
        try:
            ...
            # Limpieza y mapeo del Dataset de Entrenamiento
            ...
            # Selección de variables numéricas para entrenar
            y_train = df['Resultado']
            X_train_bruto = df.drop(columns=['Resultado'], errors='ignore')
            ...
            # Obtención de alumnos reales desde la Base de Datos para predecir
            query_completos = """
                SELECT 
            ...
            if df_db.empty:
                ...
                # Mapear y escalar las columnas correspondientes exactas
            ...
            st.error(f"❌ ERROR DETECTADO EN ENTRENAMIENTO: {str(e)}")
            st.session_state['analisis_completado'] = False
        ...
        with st.container(border=True):
            ...
            if not lista_alumnos:
                ...
                for row in lista_alumnos:
                    ...
                    st.markdown(f"<span style='color:{color}; font-weight:bold;'>{row[2]}</span>", unsafe_allow_html=True)
                    if st.button(f"🔍 Evaluar", key=f"eval_{row[0]}"):
