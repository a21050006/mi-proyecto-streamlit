...
st.markdown("""
    <style>
        ...
        /* Estilos globales para botones y tarjetas */
        .stButton>button { 
            border-radius: 8px; 
            font-weight: bold; 
        }
...
# Variables de Control de Navegación del Dashboard (Drill-Down)
...
# --- CALLBACK PARA EL PROGRESO DEL ENTRENAMIENTO DE IA ---
...
# ==========================================
# NUEVO MÓDULO: DASHBOARD INTERACTIVO (DRILL-DOWN)
# ==========================================
def mostrar_dashboard_interactivo():
    ...
    # --- RECATEGORIZACIÓN INTELIGENTE A 3 ESTADOS ---
    ...
    # ------------------------------------------
        ...
        if 'Riesgo' in estado or 'Crítico' in estado:
            st.markdown("#### 📉 Factores Críticos Específicos que inciden en el fallo")
            ...
            if vars_criticas:
                ...
                fig_lol.update_layout(
                    xaxis_title="Frecuencia (Alumnos afectados)",
                    yaxis_title="Variable Académica o Personal",
            ...
            st.success("✅ Los alumnos mantienen un estatus óptimo dadas sus sólidas métricas base:")
    ...
    # ------------------------------------------
        ...
        if not df_final.empty:
            ...
            st.dataframe(df_vista, use_container_width=True, height=350)
            
            csv = df_final[columnas_vista].to_csv(index=False).encode('utf-8')
            st.download_button(
                label="📥 Exportar Lista Nominal a CSV",
                data=csv,
...
# --- PANTALLA DOCENTE / ADMINISTRADOR ---
def pantalla_docente_admin():
    # Barra lateral de navegación superior
    ...
    # Manejo de pestañas guardado en estado para preservar consistencia al regresar de niveles
    tabs_nombres = [
    ...
    indice_defecto = tabs_nombres.index(st.session_state['tab_actual'])
    
    tab1, tab2, tab3, tab4 = st.tabs(tabs_nombres)
    
    # Sincronización manual de la selección de tabs
    # (Nota: Streamlit maneja el renderizado interno; forzamos el contenido correspondiente)
    
    # ------------------------------------------
    # PESTAÑA 1: EJECUTAR DIAGNÓSTICO IA
    # ------------------------------------------
    with tab1:
        st.session_state['tab_actual'] = "🚀 Ejecutar Diagnóstico"
        ...
        if st.button("⚡ Iniciar Diagnóstico Predictivo", type="primary"):
            ...
            # Fallback a generación sintética robusta en caso de no conectar a base de datos
            if df_sql is None:
                ...
                for i in range(150):
                    ...
                    # Generación lógica coherente de factores de riesgo
            ...
            # --- PROCESO DE ENTRENAMIENTO DE IA (TensorFlow) ---
            st.info("🏋️ Preprocesando variables académicas y estructurando sets de entrenamiento...")
            
            # Variable objetivo sintética para entrenamiento basada en el rendimiento
            ...
            # Estructura del Modelo Secuencial Inteligente
            model = Sequential([
                Input(shape=(4,)),
            ...
            # Predicción final de probabilidades exactas
            pred_probs = model.predict(X_scaled_all).flatten()
            
            # --- CONSTRUCCIÓN DE ATRIBUTOS EXPLICATIVOS ---
            factores_criticos = []
            ...
            st.success("🎉 ¡Modelo optimizado con éxito! Los datos analíticos se cargaron en la pestaña del Dashboard.")
            time.sleep(1)
            st.rerun()
            
        if st.session_state['analisis_completado'] and st.session_state['df_resultados'] is not None:
            st.write("")
            st.success("📊 Existe un diagnóstico activo en la memoria del servidor. Puedes consultar los detalles nominales en las siguientes pestañas.")
            st.dataframe(st.session_state['df_resultados'].head(5), use_container_width=True)

    # ------------------------------------------
    # PESTAÑA 2: DASHBOARD INTERACTIVO (Plotly)
    # ------------------------------------------
    with tab2:
    ...
    # ------------------------------------------
    # PESTAÑA 3: CARGA LA INFORMACIÓN DEL ALUMNO
    # ------------------------------------------
    with tab3:
        ...
        with st.form("form_registro_alumno"):
            ...
            if btn_guardar_alumno:
                if not mat_form or not nom_form:
                    ...
                    # Intento de inserción en base de datos local
                    conn = conectar_bd()
                    ...
                    if guardado_exitoso:
                        st.success(f"¡Registro del Alumno {nom_form} insertado y sincronizado en MySQL!")
                    else:
                        st.success(f"¡Registro del Alumno {nom_form} guardado localmente de forma temporal!")
                        
                    # Limpiamos variables de preselección para evitar ciclos infinitos
                    st.session_state['alumno_seleccionado_evaluar'] = None
                    time.sleep(1)
                    st.rerun()

    # ------------------------------------------
    # PESTAÑA 4: ALUMNOS PENDIENTES
    # ------------------------------------------
    with tab4:
        ...
        # Simulación de lista pendiente para propósitos visuales fluidos
        lista_pendientes = [
...
# --- ENRUTAMIENTO GENERAL DEL SISTEMA ---
